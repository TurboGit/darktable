...
static void use_module_border_callback(GtkWidget *widget,
                                       gpointer user_data)
{
  dt_conf_set_bool("themes/module-border",
                   gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(widget)));
                             ...
                             dt_gui_themetweak_widgets_t *tw)
{
  ...
  // module border
  GtkWidget *t_border = gtk_check_button_new();
  label = gtk_label_new(_("module border"));
  gtk_widget_set_name(label, "theme-variant");
  gtk_widget_set_halign(label, GTK_ALIGN_START);
  labelev = gtk_event_box_new();
  gtk_widget_add_events(labelev, GDK_BUTTON_PRESS_MASK);
  gtk_container_add(GTK_CONTAINER(labelev), label);
  gtk_grid_attach(GTK_GRID(grid), labelev, 0, line++, 1, 1);
  gtk_grid_attach_next_to(GTK_GRID(grid), t_border, labelev, GTK_POS_RIGHT, 1, 1);
  gtk_widget_set_tooltip_text(t_border, _("use a border for expanded/focuses modules"));
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(t_border),
                               dt_conf_get_bool("themes/module-border"));
  g_signal_connect(G_OBJECT(t_border), "toggled",
                   G_CALLBACK(use_module_border_callback),
                  (gpointer)t_border);
